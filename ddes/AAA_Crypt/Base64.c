...
int BASE64_decode(uint8_t *out, int out_size, const uint8_t *in_str,int in_len)
{
    uint8_t *dst = out;
    ...
    while (end - dst > 3 && (in - in_str <= in_len))
    {
        BASE64_DEC_STEP(0);
        BASE64_DEC_STEP(1);
        BASE64_DEC_STEP(2);
...
uint8_t *BASE64_encode(uint8_t *out, int out_size, const uint8_t *in, int in_size)
{
    static const char b64[] =
        "ABCDEFGHIJKLMNOPQRSTUVWXYZabcdefghijklmnopqrstuvwxyz0123456789+/";
    uint8_t *ret=NULL, *dst=NULL;
    unsigned i_bits = 0;
    int i_shift = 0;
    int bytes_remaining = in_size;

    if (in_size >= INT_MAX / 2 || 
